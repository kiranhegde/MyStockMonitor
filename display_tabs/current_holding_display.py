...
from gui_widgets.gui_widgets_for_adding_new_stock import \
    add_new_stock as new_stock
from gui_widgets.gui_widgets_for_editing_selected_stock import \
    edit_selected_stock
from gui_widgets.gui_widgets_for_selling_selected_stock import \
    sell_selected_stock
...
class list_of_holdings_display(QWidget):
    ...
    def layouts(self):
        ...
        # self.plot_graphs()
        self.plot_graphs(moving_avg='SMA', price=row_data[3],
    ...
    def plot_graphs(self, moving_avg, price=100, buy_date=datetime.date.today()
                    , symbol="DMART"):
        ...
        # exit()
        if symbol_buy_date in self.current_holding_history.keys() and not \
                self.current_holding_history[
    ...
    def export2excel_casesheet(self):
        ...
        if not excel2report_df.isnull().values.all():
            excel2report_df['Date'] = excel2report_df['Date'].dt.strftime(
                DATE_FMT_DMY)
        ...
        excel2report_df = excel2report_df.append(classsum1,
                                                 ignore_index=True).fillna(" ")
        # print(excel2report_df.to_string())
        last_row = classsum1.to_dict()
        # print(last_row)

        grand_total = parse_str(last_row['Unpaid']) - (
                parse_str(last_row['Cash']) +
        ...
        excel2report_df = excel2report_df.append(classsum2,
                                                 ignore_index=True).fillna(" ")
        # print("#", grand_total)
        ...
        file_path, ext = QFileDialog.getSaveFileName(self, "Save As", fpath,
                                                     "(*.xlsx)",
                                                     options=options)
        # print(file_path, ext)
            ...
            if file_path != "":
                try:
                    ...
                    with writer_to_excel as writer:
                        excel2report_df.to_excel(writer,
    ...
    def rename_casesheet(self):
        ...
        if item_name == "Others":
            ...
            messge = self.total_holdings_details.update_rows_by_column_values(
                set_argument=pname_mobile,
    ...
    def clear_casesheet(self):
        ...
        if balance_amount > 0.0:
            ...
            msgBox.setText(
                f"Are you sure to clear transaction information of {item_name} from unpaid ?")
    ...
    def rightClickMenuPayIn(self, pos):
        ...
        if not mdlIdx.isValid() and len(indexes) != 0:
            ...
            newAct = QAction(QIcon(""), "New", self,
                             triggered=self.add_new_stock)
            editeAct = QAction(QIcon(""), "Edit Bill", self,
                               triggered=self.edit_selected_stock)
            ...
            soldAct = QAction(QIcon(""), 'Sold', self,
                              triggered=self.stock_sold)
            avgAct = QAction(QIcon(""), 'Average', self,
                             triggered=self.stock_avg)
            # CloseBalanceAct = QAction(QIcon(""), "Clear Balance and Close a/c", self, triggered=self.clear_and_close)
    ...
    def plot_history(self, moving_avg='SMA'):
        ...
        # print(row_data)
        self.plot_graphs(moving_avg, price=row_data[3], buy_date=row_data[2],
                         symbol=row_data[1])
    ...
    def add_new_stock(self):
        ...
        if newBill_inp.exec_() == newBill_inp.Accepted:
            ...
            for hdr_name in CURRENT_HOLDINGS_HEADER_DISPLAY_LIST:
                CURRENT_HOLDINGS_HEADER_DB.append(
                    CURRENT_HOLDINGS_HEADER_DISPLAY_LIST2DB[hdr_name])
            df_row = df_row.loc[:, CURRENT_HOLDINGS_HEADER_DB]
            df_row.columns = CURRENT_HOLDINGS_HEADER_DISPLAY_LIST

            self.current_holding_data_df = pd.concat(
                [df_row, self.current_holding_data_df]).reset_index(drop=True)
            self.holdingModel.update(self.current_holding_data_df, key="New")
            # self.holdingModel.layoutChanged.emit()
            # self.holdingTable.clearSelection()

            if save_db:
                # date_time = datetime.datetime.strptime(new_stock_addition['date'], DATE_FMT_DMY)
                # new_stock_addition['date'] = date_time.date()
                listt = list(new_stock_addition.values())
                vals_tuple = [tuple(listt)]
                messge = self.total_holdings_details.insert_row_by_column_values(
                    row_val=vals_tuple)
    ...
    def modify_stock_details(self, balance_amount=None):
        ...
        row_data_db = self.total_holdings_details.read_row_by_column_values(
        ...
        if row_data_db['ref_number'] != '':
            ...
            if editBill_inp.exec_() == editBill_inp.Accepted:
                ...
                # pd.to_datetime(edit_stock_details[''], format="%d-%m-%y")
                mask1 = self.current_holding_data_df["Reference"] == \
                        row_data_db['ref_number']
                for k, v in edit_stock_details.items():
                    ...
                    if key in self.current_holding_data_df.columns.values.tolist() and key != 'Reference':
                        self.current_holding_data_df.loc[mask1, key] = \
                            edit_stock_details[k]

                self.holdingModel.update(self.current_holding_data_df,
                                         key="New")
                self.holdingModel.layoutChanged.emit()
                self.holdingTable.clearSelection()
                ...
                if save_db:
                    set_list = ""
                    # date_time = datetime.datetime.strptime(edit_stock_details['date'], DATE_FMT_DMY)
    ...
    def delete_selected_stock(self):
        ...
        if row_data[0] != '':
            ...
            if msgBox.clickedButton() == buttonY:
                message = self.total_holdings_details.delete_row_by_column_values(
                    criteria=search_item)
                mask1 = self.current_holding_data_df["Reference"] == int(
                    row_data[0])
    ...
    def stock_sold(self):
        ...
        if row_data_db['ref_number'] != '':
            ...
            if sell_stock_inp.exec_() == sell_stock_inp.Accepted:
                ...
                balance_quantity = int(old_quantity) - int(
                    sell_stock_details['quantity'])
                if balance_quantity > 0:
                    ...
                    if save_db:
                        ...
                        messge = self.total_holdings_details.insert_row_by_column_values(
                            row_val=vals_tuple)
                        # print(messge)
                        mask1 = self.current_holding_data_df[
                                    "Reference"] == int(row_data[0])
                        index = self.current_holding_data_df[mask1].index
                        self.current_holding_data_df.drop(index, axis=0,
                                                          inplace=True)
                        self.holdingModel.update(self.current_holding_data_df,
                                                 key="Delete")
                        self.holdingModel.layoutChanged.emit()
    ...
    def clear_and_close(self):
        ...
        if balance_amount > 0.0:
            self.modify_stock_details(BILL_MODIFY_TYPE[3],
    ...
    def move_to_unpaid_payin(self):
        ...
        if row_data[1] != '' and row_data[3] != '':
            ...
            row_data_db = self.total_holdings_details.read_row_by_column_values(
                criteria=date_time_bill_no)[0]
            check_id = f"{row_data_db['patient_name']}_{row_data_db['mobile_no']}"
            items0 = [self.Holding_List.item(x).text() for x in
                      range(self.Holding_List.count())]
            ...
            if row_data_db['pay_unpaid'] == 0:
                ...
                if name_mobile_inp.exec_() == name_mobile_inp.Accepted:
                    ...
                    self.df_reception_payin.loc[
                        index, ["Patient Name"]] = pnameNew
                    self.holdingModel.update(self.df_reception_payin, key="New")
                    self.holdingModel.layoutChanged.emit()
                    self.holdingTable.clearSelection()
                    id = f"{pnameNew}_{mobile_number}"

                    items = [self.Holding_List.item(x).text() for x in
                             range(self.Holding_List.count())]
                    if id not in items:
...
if __name__ == '__main__':
    ...
    super_trend = list_of_holdings_display(**mysql_data)
    # super_trend.show()
